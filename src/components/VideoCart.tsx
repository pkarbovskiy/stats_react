    ...
    return (
        <div className="video_cart">
            <Link to={`/video/${videoId}/${timestamp ? `timer/${timestamp}/` : ''}`}>
                <img className="video_cart--image" src={`https://d38ev7kpu49one.cloudfront.net/${imageId ? imageId : id}.png`} alt={title != null ? title : `${player.name}`} />
            </Link>
            <div className="video_cart__info">
                ...
                <span className="video_cart__info__description">
                    {title != null ? title : <Link to={`/player/${player.id}/${player.slug}`}>{player.name}</Link>}
                </span>
            </div>
        </div>
    )
