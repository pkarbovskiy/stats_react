...
const DeathKill = ({ deathKillTimers, videoHandler }: DeathKillProps) => {
    ...
    return (
        <nav className="video__nav">
            <button className={`video__nav--btn${state.isDeathPrevDisabled ? ' disabled' : ''} left`} onClick={() => move(0, currentAction)} disabled={state.isDeathPrevDisabled}>
                <svg width="12" height="19" viewBox="0 0 12 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 16.7394L9.70111 19L-2.07629e-06 9.5L9.70111 -5.02438e-07L12 2.26057L4.60731 9.5L12 16.7394Z"
                        fill="white" />
                </svg>
                <label className="video__nav--lbl">
                    Previous
                </label>
            </button>
            <div className="video__nav__container">
                <button className="video__nav__action" onClick={() => goToNextAction()} >
                    <VideoFilters action={currentAction} />
                </button>
            </div>
            <button className={`video__nav--btn${state.isKillNextDisabled ? ' disabled' : ''} right`} onClick={() => move(1, currentAction)} disabled={state.isKillNextDisabled}>
                <label className="video__nav--lbl">
                    Next
                </label>
                <svg width="12" height="19" viewBox="0 0 12 19" fill="none" xmlns="http://www.w3.org/2000/svg">
